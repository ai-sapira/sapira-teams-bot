...
Responde SOLO: true o false`;
  ...
  async continueConversation(conversation) {
    const prompt = `Eres Sapira, un asistente conversacional para Gonvarri que ayuda a documentar ideas de automatización e IA.

CONVERSACIÓN HASTA AHORA:
${conversation.getHistory()}

INSTRUCCIONES:
1. Lee TODA la conversación anterior y entiende el contexto
2. Responde de forma natural y específica al último mensaje del usuario
3. Haz preguntas concretas sobre lo que falta: problema específico, tecnología, beneficio esperado, o contexto
4. Sé BREVE (1-2 frases máximo)
5. NO repitas preguntas ya hechas
6. NO ignores lo que el usuario ya te contó
7. Usa español estándar (tú/ustedes)

Responde como Sapira:`;

    try {
      console.log('🔮 Gemini: Generating response for conversation with', conversation.messages.length, 'messages');
      const model = this.client.getGenerativeModel({ model: "gemini-2.0-flash-exp" });
      const response = await model.generateContent(prompt);
      const text = response.response.text().trim();
      console.log('🔮 Gemini: Generated response:', text.substring(0, 100));
      return text;
    } catch (error) {
      console.error('❌ Gemini error in continueConversation:', error);
...
{
  "title": "NOMBRE DE PROYECTO/PRODUCTO (máx 40 chars, 2-3 palabras - ver reglas abajo)",
...
1. ❌ NUNCA empieces con: "This initiative...", "Automated...", "System for...", "Tool for...", "Solution for..."
2. ✅ USA un NOMBRE DE PROYECTO/PRODUCTO directo: "InvoiceAutoFlow", "FraudFinder AI", "GMHS Offer Bot"
3. ❌ NO descripciones: "Automated invoice processing system"
4. ✅ SÍ nombres creativos/específicos: "InvoiceGenius", "CareerPath AI"
5. Máximo 2-3 palabras, 40 caracteres
6. Debe ser memorable y específico a la propuesta de la conversación

EJEMPLOS DE TÍTULOS CORRECTOS E INCORRECTOS:

Conversación sobre automatizar facturas:
  ❌ "This initiative automates invoice processing"
  ❌ "Automated invoice system"
  ❌ "Invoice Automation"
  ✅ "InvoiceAutoFlow"
  ✅ "FinanceFlow AI"

Conversación sobre chatbot para RRHH:
  ❌ "This initiative addresses HR queries"
  ❌ "HR Chatbot with AI"
  ✅ "CareerPath AI"
  ✅ "HRAssist Bot"

Conversación sobre detectar fraude:
  ❌ "System for fraud detection"
  ❌ "Fraud Detection Tool"
  ✅ "FraudFinder AI"
  ✅ "ReceivablesGuard"

Conversación sobre automatizar ofertas GMHS:
  ❌ "This initiative aims to automate offers"
  ❌ "Offer automation system"
  ✅ "GMHS Offer Bot"
  ✅ "OfferCrafter AI"
...
REGLAS FINALES:
- title: NOMBRE DIRECTO, no descripción. Máximo 40 caracteres, 2-3 palabras. ¡NO empieces con "This"!
