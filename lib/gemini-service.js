...
⚠️ CRITERIO MUY ESTRICTO:

✅ SUFICIENTE SI (TODOS los puntos):
1. El usuario explicó QUÉ proceso/problema ESPECÍFICO quiere resolver (no "las facturas", sino "validar facturas en SAP")
2. El usuario mencionó EXPLÍCITAMENTE la tecnología o enfoque (RPA, IA, GenAI, IDP, Analytics, etc.)
3. Se mencionó VOLUMEN o FRECUENCIA del problema (cuántas facturas, cuánto tiempo, etc.)
4. Se mencionó el beneficio/impacto CUANTIFICADO (ahorrar X horas, reducir Y errores, etc.)
5. Se mencionó quién está involucrado (qué departamento, cuántas personas, etc.)
6. Hay suficiente detalle técnico para escribir una descripción de 2-3 párrafos con sentido

❌ INSUFICIENTE SI (cualquiera):
- Solo hubo saludos o mensajes muy vagos
- El usuario solo mencionó el problema sin detalles técnicos
- Falta el QUÉ EXACTO (solo "facturas" no es suficiente, necesito "validar campos X, Y en sistema Z")
- Falta el CÓMO (si no se mencionó tecnología explícita, NO es suficiente)
- Falta contexto de VOLUMEN (cuántas facturas/día, cuánto tiempo consume, etc.)
- La conversación es ambigua o abstracta
- El usuario todavía está explorando sin una idea clara
- Solo hay 3-4 intercambios de mensajes
...
RESPUESTA: false (falta QUÉ EXACTAMENTE se automatiza, falta CÓMO (tecnología), falta VOLUMEN, muy genérico)

❌ INSUFICIENTE:
user: "Necesitamos automatizar algo en RRHH"
bot: "¿Qué proceso específico?"
user: "Las nóminas, es muy tedioso"
bot: "¿Cuánto tiempo os lleva?"
user: "Mucho"
RESPUESTA: false (sin detalles técnicos, sin tecnología, "mucho" no es cuantificable)
...
user: "Ahorrar tiempo, ahora el equipo de 3 personas pasa 15 horas semanales en esto"
bot: "¿Habéis pensado en alguna tecnología específica?"
user: "Sí, algo con IDP para leer las facturas y RPA para meter los datos en SAP"
RESPUESTA: true (problema específico, volumen cuantificado, tecnología explícita, beneficio medible, equipo identificado)
...
Responde SOLO: true o false`;
    ...
    try {
      ...
      // ⭐ NUEVO: Fallback MUY conservador
      // Solo crear si hay MUCHOS mensajes (12+) asumiendo que ya hubo conversación MUY larga
      const shouldCreate = messageCount >= 12;
...
Entender la idea/problema del usuario para ayudarle a crear una initiative bien documentada. Necesitas captar TODO esto:

✅ INFORMACIÓN CRÍTICA QUE NECESITAS:
1. El problema o proceso ESPECÍFICO (no "facturas", sino "validar facturas de proveedores en SAP")
2. La tecnología o enfoque EXPLÍCITO (RPA, GenAI, IDP, Analytics, ML, etc.)
3. El VOLUMEN o FRECUENCIA (cuántas facturas/día, cuántas horas/semana, etc.)
4. El beneficio CUANTIFICADO (ahorrar X horas, reducir Y errores, eliminar Z tareas)
5. Los SISTEMAS involucrados (SAP, Oracle, Excel, email, etc.)
6. El EQUIPO afectado (qué departamento, cuántas personas)

⚠️ REGLAS DE ORO:
- NO hagas preguntas mecánicas tipo checklist
- SI el usuario SOLO saluda → devuelve saludo + pregunta abierta simple
- SI el usuario da info VAGA → pide detalles CONCRETOS (números, sistemas, tecnología)
...
✅ Bueno: "Interesante. ¿Cuántas facturas recibís al día o al mes más o menos?"
...
✅ Bueno: "Claro. ¿Me puedes dar más detalle? Por ejemplo, ¿qué parte del proceso es la más manual? ¿Meter datos en algún sistema?"
❌ Malo: "Genial, ¿y qué más?"

Usuario: "Unas 200 facturas al mes"
✅ Bueno: "Vale, 200 al mes es un buen volumen. ¿Ahora cómo las procesáis? ¿Las tenéis que meter en SAP o en algún otro sistema?"
❌ Malo: "Perfecto. ¿Algo más?"

Usuario: "Sí, las metemos en SAP manualmente"
✅ Bueno: "Entiendo. ¿Cuánto tiempo os lleva eso? Y, ¿habéis pensado en alguna tecnología tipo IDP para leer las facturas o RPA para automatizar la entrada en SAP?"
❌ Malo: "Ok, ¿qué beneficio esperarías?"

Usuario: "¿Tú qué piensas que sería mejor?"
✅ Bueno: "Por lo que me cuentas, algo con IDP para leer las facturas y RPA para meter los datos en SAP podría funcionar bien. ¿Cuánto tiempo estimáis que os ahorraría?"
